    ...
    <main class="container bg-primary text-white flex-1">
        <div class="pt-4 mb-8 relative">
            <input 
                ...
                class="py-2 px-1 w-full text-black border-b
                focus:border-secondary focus:outline-none 
                focus:shadow-[0px_1px_0_0_#004E71]"
            >
            <!-- Insert unordered list -->
            <ul
                class="absolute bg-secondary text-white w-full shadow-md py-2 px-1 top-[66x]"
                v-if="mapboxSearchResults"
            >
                <p class="pyo-2" v-if="searchError">
...
const previewCity = (searchResult) => {
    ...
    //This will add out new city and state to our router object
    router.push({
        name: "city",
...
const mapboxKey = 'pk.eyJ1IjoiZG9ub3Zhbm9saW5uIiwiYSI6ImNsZHc3bnhxdTA0dTYzb25vamc0dmZlangifQ.VZQ1EjGb2ViwXqwQKqoj0Q'
...
const getSearchResults = () => {
    ...
    queryTimeout.value = setTimeout(async () => {
        if (searchQuery.value !== "") {
            try {
                // url/v5/endpoint/${searth terms}/${api key}/${ optional extra parameters}
